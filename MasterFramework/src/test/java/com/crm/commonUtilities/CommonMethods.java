...
public static boolean isTestRunnable(String testName, String sheetName) throws Exception
{
	...
	public static void SelectFromList(String Locator,String sheetName, String columnName) throws Exception
    {
        try {
            List<WebElement> WebelementsList = driver.findElements(By.xpath(prop.getProperty(Locator)));
            String excelVal=ExcelOperation.getCellData(sheetName, columnName, 1);
            ExWaitsForWebelements(WebelementsList);
            for(WebElement ele:WebelementsList)
            {
                String ElementText=ele.getText();
                if(ElementText.equalsIgnoreCase(excelVal))
                {
                    mouseClick(ele);
                    log.info("Selected Value :"+excelVal);
                    break;
                }
            }
        } catch (Exception e) {
            log.error("WebElements list handled exception occured due to "+e.getMessage());
        }
